...
def widget_ID():
    ...
    frame_participantID = tk.Frame(second_screen_window)
...
def instructions():
    ...
    frame_button_next = tk.Frame(second_screen_window)
...
def start_screen():
    ...
    frame_button_start = tk.Frame(second_screen_window)
...
def countdown(minutes, seconds):
    ...
    if minutes > 0 or seconds > 0:
        if seconds == 0:
            ...
            second_screen_window.after(1000,countdown, minutes - 1, 59)
        else:
            # Countdown to the next second
            second_screen_window.after(1000, countdown, minutes, seconds - 1)
        ...
        ######################################################
        second_screen_window.after(120000, loop_countdown)
...
if len(monitors) > 1:
    # If there is a second screen, get its dimensions
    second_monitor = monitors[1]
    screen_width = second_monitor.width
    screen_height = second_monitor.height

    # Determine the size of the window
    window_width = min(screen.winfo_screenwidth(), screen_width)
    window_height = min(screen.winfo_screenheight(), screen_height)

    # Create a new Tkinter window
    second_screen_window = tk.Toplevel(screen)

    # Position and size the window to fit the second screen
    window_x = second_monitor.x
    window_y = second_monitor.y
    second_screen_window.geometry(f"{window_width}x{window_height}+{window_x}+{window_y}")
